...
def clamp(val: float, bounds: Tuple[float, float] | None) -> float:
    if not bounds:
        return val
    lo, hi = bounds
    if val < lo or val > hi:
...
def map_value_ik(value, in_min, in_max, out_min, out_max):
    """Maps a value from one range to another (like Arduino map)."""
    return (value - in_min) * (out_max - out_min) / (in_max - in_min) + out_min


def map_range_adj(val: float, src: Tuple[float, float], dst: Tuple[float, float], invert: bool = False) -> float:
    s0, s1 = src
    d0, d1 = dst
...
class GR2Mimic(Node):
    ...
    # --- DATA RECORDING SETUP ---
        self.frame_count = 0
        self.data_log = [] # List to store data rows
        
        # Define which logic keys you want to graph
        # These keys must exist in your JOINTS dictionary
    ...
    def optimize_joint(self, joint_name, human_data_array, robot_limits):
        ...
        def fitness(params):
            ...
            # 2. Calculate Clipping            
            clipped_mask = (t < 0) | (t > 1)
            clipping_penalty = np.sum(clipped_mask) / len(t) # Percent of frames clipped
            
            # 3. Calculate Range Usage           
            spread = np.std(t)
            spread_score = (0.3 - spread)**2 # Penalty if spread is too low
    ...
    def run_calibration(self):
        ...
        for key, value in JOINTS.items():             
            ...
            if key if isinstance(value, list) else value in df.columns: 
                ...
                print(limit)                   
                # Check variance to ensure we actually have data (not just 0.0s)
                if df[item].std() < 0.01:                   
    ...
    def timer_callback(self) -> None:
        ...
        for name in self.joint_names:
            ...
            self.current_pos[name] = clamp(smoothed, JOINT_LIMITS.get(name))
    ...
    def extract_body_targets(self, frame: np.ndarray, w: int, h: int) -> Dict[str, float]:
        ...
        for side in ("left", "right"):
            ...
            roll_deg = angle_between(base - shoulder, elbow - shoulder)    
            ...
            if self.apply_optimization:
                ...
                roll = map_range_adj(roll_deg, (best_src[0], best_src[1]), (-0.1, 2.5 * roll_sign))
            else:
                roll = map_range_adj(roll_deg, (20, 100), (-0.1, 2.5 * roll_sign))
            ...
            if self.apply_optimization: 
                ...
                pitch = map_range_adj(pitch_offset, (best_src[0], best_src[1]), (0.0, -1.2), invert=False)    
            else:
                pitch = map_range_adj(pitch_offset, (-1, 1), (0.0, -1.2), invert=False) 
            ...
            if side == "left":   
                if self.apply_optimization: 
                    ...
                    yaw = map_range_adj(yaw_offset, (best_src[0], best_src[1]), (-0.3, 1.2), invert=False)
                else: 
                    yaw = map_range_adj(yaw_offset, (0.1, 0.6), (-0.3, 1.2), invert=False)
            else:     
                if self.apply_optimization: 
                    best_src = self.optimize_joints.get(f"{side}_shoulder_yaw_joint", (0.0, 1.0))            
                    yaw = map_range_adj(yaw_offset, (best_src[0], best_src[1]), (-1.2, 0.3), invert=False)
                else: 
                    yaw = map_range_adj(yaw_offset, (-0.6, -0.1), (-1.2, 0.3), invert=False)
            ...
            if self.apply_optimization: 
                ...
                elbow_rad = map_range_adj(elbow_deg, (best_src[1], best_src[0]), (0.4, -1.5), invert=False)          
            else:   
                elbow_rad = map_range_adj(elbow_deg, (170, 10), (0.4, -1.5), invert=False)
    ...
    def hand_to_targets(self, kpts: np.ndarray, frame: np.ndarray, side: str, frame_width: int, frame_height: int) -> Dict[str, float]:
        ...
        if self.apply_optimization:
            ...
            roll = map_range_adj(roll_angle, (best_src[0], best_src[1]), (0.8 * sign, -0.8 * sign), invert=False)
        else: 
            roll = map_range_adj(roll_angle, (-2.5, 2.5), (0.8 * sign, -0.8 * sign), invert=False)
        #out[roll_joint] = roll
        ...
        if self.apply_optimization:
            best_src = self.optimize_joints.get(f"{side}_wrist_pitch_joint", (0.0, 1.0)) 
            pitch = map_range_adj(rel_y, (best_src[0], best_src[1]), (0.0, 0.6))
        else:
            pitch = map_range_adj(rel_y, (-2.0, -0.8), (0.0, 0.6))
        out[pitch_joint] = pitch
        ...
        if self.apply_optimization:
            ...
            yaw = map_range_adj(spread_angle, (best_src[0], best_src[1]), (-1.4 * sign, 1.4 * sign))
        else:
            yaw = map_range_adj(spread_angle, (-1.5, 1.5), (-1.4 * sign, 1.4 * sign))
        #out[yaw_joint] = yaw
        ...
        if self.apply_optimization:
            ...
            thumb_val = map_range_adj(thumb_ratio, (best_src[1], best_src[0]), (1.0, 0.0))
        else:
            thumb_val = map_range_adj(thumb_ratio, (0.5, 1.5), (1.0, 0.0))
        ...
        for fname, (mcp_key, pip_key, dip_key, tip_key) in finger_sets.items():
            ...
            if self.apply_optimization:
                ...
                curl_val = map_range_adj(ratio, (best_src[1], best_src[0]), (0.0, -1.5)) 
            else:
                curl_val = map_range_adj(ratio, (1.6, 1.0), (0.0, -1.5))     
